...
func UpdateReaction(likeOrDislike bool, user_id int,  post_id int) {
	_, err := DB.Exec("UPDATE likes_dislikes SET is_like = ? WHERE user_id = ? AND post_id = ?", likeOrDislike, user_id, post_id)
	if err != nil{
		log.Printf("Error updating likes/dislike: %v", err)
		return 
	}
}